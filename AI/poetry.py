...
if __name__ == "__main__":
    ...
    words, _ = zip(*count_pairs)  # zip(*xxx)是zip的反操作

    # 诗向量化
    words = words[:len(words)] + (' ',)    # 取前多少个常用字，在这里都取了
    word_num_map = dict(zip(words, range(len(words)))) # 每个字映射为一个数字ID
    to_num = lambda word: word_num_map.get(word, len(words))
    poetrys_vector = [list(map(to_num, poetry)) for poetry in poetrys] # 把诗转换为向量形式，参考TensorFlow练习1；有默认值的get

    # batch_size为64, 意味着每次用64首诗进行训练
    batch_size = 64
    total_batches = len(poetrys_vector) // batch_size
    ...
    with tf.Session(config=Session_config) as sess:
        with tf.device('/gpu:0'):
            ...
            last_epoch = load_model(sess, saver, 'poetry_model/')
            ...
            for epoch in range(last_epoch + 1, 100):
                ...
                saver.save(sess, 'poetry_model/model.ckpt', global_step=epoch)
                print(epoch, ' Loss: ', all_loss * 1.0 / total_batches)